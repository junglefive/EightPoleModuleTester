...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def on_clicked_btn_iap_get_version(self):
        self.CMD_CSM37F58_IAP_GET_VERSION_ADDRESS = 0xFFFF
        print("获取版本")
        try:
            protocol = CH341AIIC()
    ...
    def iic_send_bin_file(self,bin_data):
        ...
        try:
            ...
            last_frame = bin_data[(frame_cnt)*512:]
            ...
            for i in range(len(last_frame)):
                last[i] = last_frame[i]
            protocol.write_iap_bytes(frame_cnt*512,last)
            print(("发送last: "+bytes(last).hex()))
        #     send checksum
            protocol.write_iap_bytes(0xEC01,self.iap_checksum)
    ...
    def on_clicked_btn_iap_loadfile(self):
        ...
        try:
            ...
            # last
            #
            self.iap_checksum[5+frame_cnt] = checksum&0xff
            self.iap_checksum[4] = frame_cnt+1
            self.textBrowser_iap.append("第%s包(512):" % (frame_cnt + 1))
            ...
            for i in range(512):
                checksum =checksum + self.iap_checksum[i]
            self.iap_checksum[511] = checksum&0xff

            self.textBrowser_iap.append("第%s包(校验码):" % (frame_cnt + 2))
